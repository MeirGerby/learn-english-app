      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 pt-4 flex flex-col gap-6">
        ...
        <main className="max-w-md w-full mx-auto mt-2">
          ...
          <div className="relative overflow-hidden rounded-3xl bg-white border border-slate-200 shadow-sm dark:bg-slate-900/80 dark:border-slate-800/80 dark:backdrop-blur-xl dark:shadow-2xl p-6 sm:p-8">
            ...
            {mode === "reset" ? (
              <form onSubmit={handleResetSubmit} className="flex flex-col gap-4">
                <div>
                  <Label htmlFor="reset-email" className="text-slate-600 dark:text-slate-300 text-xs font-medium mb-1.5 block">
                    דואר אלקטרוני
                  </Label>
                  <Input
                    id="reset-email"
                ...
                {resetForm.message && (
                  <p aria-live="polite" className="text-teal-600 dark:text-teal-400 text-xs mt-0.5 font-medium">
                    {resetForm.message}
                  </p>
                )}
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-1.5 text-xs text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white mt-2 transition-colors"
                >
              ...
              <form onSubmit={handleLoginSubmit} className="flex flex-col gap-4">
                <div>
                  <Label htmlFor="login-email" className="text-slate-600 dark:text-slate-300 text-xs font-medium mb-1.5 block">
                    דואר אלקטרוני
                  </Label>
                  <Input
                    id="login-email"
          ...
          <p className="text-center mt-6 text-slate-500 dark:text-slate-400 text-xs">
            אין לכם חשבון עדיין?{" "}
            <Link to="/register" state={location.state} className="text-rose-600 dark:text-rose-400 font-bold hover:underline">
              הרשמה למערכת
            </Link>
          </p>
        </main>
