      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 pt-4 flex flex-col gap-6">
        ...
        <main className="max-w-md w-full mx-auto mt-2">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">איפוס סיסמה</h1>
          </div>

          {!token ? (
            <div className="relative overflow-hidden rounded-3xl bg-white border border-slate-200 shadow-sm dark:bg-slate-900/80 dark:border-slate-800/80 dark:backdrop-blur-xl dark:shadow-2xl p-8 text-center">
              <AlertTriangle className="w-12 h-12 text-amber-500 dark:text-amber-400 mx-auto mb-3" />
              <h2 className="font-bold text-lg text-slate-900 dark:text-white mb-2">קישור לא תקין</h2>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-6">
                הקישור לאיפוס הסיסמה חסר או שגוי. בקשו קישור חדש מדף ההתחברות.
              </p>
              <Link to="/login">
                <Button className="bg-slate-100 hover:bg-slate-200 text-slate-900 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-white font-bold rounded-xl">
                  חזרה להתחברות
                </Button>
              </Link>
            </div>
            ...
            <div className="relative overflow-hidden rounded-3xl bg-white border border-slate-200 shadow-sm dark:bg-slate-900/80 dark:border-slate-800/80 dark:backdrop-blur-xl dark:shadow-2xl p-8 text-center">
              <CheckCircle2 className="w-12 h-12 text-teal-500 dark:text-teal-400 mx-auto mb-3" />
              <h2 className="font-bold text-lg text-slate-900 dark:text-white mb-2">הסיסמה עודכנה בהצלחה! 🎉</h2>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-6">אפשר להתחבר עכשיו עם הסיסמה החדשה.</p>
              <Link to="/login">
            ...
            <div className="relative overflow-hidden rounded-3xl bg-white border border-slate-200 shadow-sm dark:bg-slate-900/80 dark:border-slate-800/80 dark:backdrop-blur-xl dark:shadow-2xl p-6 sm:p-8">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div>
                  <Label htmlFor="reset-new-password" className="text-slate-600 dark:text-slate-300 text-xs font-medium mb-1.5 block">
                    סיסמה חדשה
                  </Label>
                  <div className="relative" dir="ltr">
                    <Input
                      ...
                      className="h-11 bg-white border-slate-200 text-slate-900 placeholder:text-slate-400 dark:bg-slate-950/60 dark:border-slate-800 dark:text-white dark:placeholder:text-slate-500 focus:border-rose-500 focus:ring-rose-500/20 pe-10"
                      value={newPassword}
                    ...
                    <button
                      ...
                      className="absolute top-1/2 -translate-y-1/2 end-2 min-w-8 min-h-8 flex items-center justify-center rounded-full text-slate-400 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-slate-800 transition-colors"
                    >
                ...
                <div>
                  <Label htmlFor="reset-confirm-password" className="text-slate-600 dark:text-slate-300 text-xs font-medium mb-1.5 block">
                    אימות סיסמה
                  </Label>
                  <Input
