...
24. `[ARCH]` The "Learn English App — Additional Operating Rules" (§43 below, added 2026-08-18) mandates TypeScript/React for all new code, which the existing codebase does not use at all (it is 100% vanilla HTML/CSS/JS with no build step, deployed directly to GitHub Pages). This is an unresolved conflict: applying §43 literally to the next edit of any existing file would require migrating that file off vanilla JS. Until the user explicitly approves a real TypeScript/React migration (a large, disruptive undertaking - new build tooling, a changed deployment pipeline, and a rewrite of every existing page), treat §43's TypeScript mandate as binding for genuinely new, separate subsystems only, and continue extending the existing vanilla JS app in vanilla JS to avoid a half-migrated, inconsistent codebase. Flag this conflict again if it becomes newly relevant rather than silently picking a side.

25. `[DATA]` The word bank moved from being purely in-memory (`data.js`) to Firestore (`words` collection, public read / admin write) as the source of truth, added 2026-08-18. `words-db.js`'s `loadWords(category)` fetches from Firestore first and falls back to `data.js` (exposed as `window.WORD_DATA` for module scripts to reach) if Firestore is empty, errors, *or hangs* - `getDocs()`/`getDoc()` can hang indefinitely rather than reject (seen in this dev environment with broken IndexedDB, plausible for some real users too), so both `words-db.js` and `user-stats.js` race Firestore calls against a 5s timeout rather than relying on try/catch alone. Words are seeded into Firestore via a one-time (re-runnable/idempotent) "Import words" button in `admin.html`/`admin.js` that only an admin can run - do not attempt to seed Firestore directly via REST using a non-admin credential, `firestore.rules` blocks it by design.

26. `[DATA]` Per-user cumulative stats/achievements live in Firestore `userStats/{uid}` (owner-only read/write), managed by `user-stats.js` - added 2026-08-18. This is additive to, not a replacement for, the existing localStorage-based per-session score/streak shown in each page's header (unchanged, works for anonymous users too); the cumulative Firestore stats and unlocked-achievement badges are shown separately on the home page for logged-in users only. `recordAnswer`/`recordGameCompleted` calls in game scripts are fire-and-forget (never `await`ed) specifically so a stalled Firestore write never blocks gameplay.

27. `[UX]` Advanced-level content (idioms, phrasal verbs, sophisticated vocabulary in the `idioms`/`phrasalVerbs`/`advancedVocab` categories, plus the three dedicated games `fill-blank.html`/`listening.html`/`speed-round.html`) must visibly carry the `.level-badge` "רמה מתקדמת" (advanced level) tag - user preference stated 2026-08-18 ("sign the games that they are for advanced learning"). The three advanced games restrict their category dropdown to advanced-level categories only (`words-db.js`'s `getAdvancedCategoryKeys()`); the original three games (flashcards/quiz/scramble) still show all categories including the advanced ones, unrestricted.

---

# 43. Learn English App — Additional Operating Rules

*(Added verbatim by the user on 2026-08-18. See rule 24 above for the unresolved conflict between this section's TypeScript/React mandate and the existing vanilla-JS codebase.)*
